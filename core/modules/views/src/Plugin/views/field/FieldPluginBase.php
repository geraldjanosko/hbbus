    ...
    if ($this->allowAdvancedRender()) {
      ...
      if ($this instanceof MultiItemsFieldHandlerInterface) {
        ...
        foreach ($raw_items as $count => $item) {
          // If a link is being rendered, we need to move that information to
          // the alter settings so we can manipulate the original value before
          // rendering the link.
          if (isset($item['rendered']['#type']) && $item['rendered']['#type'] == 'link') {
            $item['url'] = $item['rendered']['#url'];
            $item['make_link'] = TRUE;
            $item['rendered'] = [
              '#plain_text' => isset($item['rendered']['#title']['#context']['value']) ? $item['rendered']['#title']['#context']['value'] : $item['rendered']['#title'],
